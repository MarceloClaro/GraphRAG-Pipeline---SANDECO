...
const steps = [
  { id: PipelineStage.UPLOAD, label: '1. Ingestão' },
  { id: PipelineStage.EMBEDDINGS, label: '2. Vetores' },
  { id: PipelineStage.CLUSTERING, label: '3. Clusters' },
  { id: PipelineStage.GRAPH, label: '4. Grafo' },
  { id: PipelineStage.QUERY, label: '5. RAG Lab' },
];
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.id} className="flex flex-col items-center bg-white px-2 z-10">
              ...
              <span className={`mt-2 text-[10px] sm:text-xs font-semibold uppercase tracking-wider ${isActive ? 'text-indigo-700' : 'text-gray-400'}`}>
                {step.label}
              </span>
            </div>
          );
